<script lang="ts">
  ...
  export let raysOrigin: RaysOrigin = 'top-center';
  export let raysColor: string = '#ffffff';
  export let raysSpeed: number = 1;
  export let lightSpread: number = 1;
  export let rayLength: number = 2;
  export let pulsating: boolean = false;
  export let fadeDistance: number = 1.0;
  export let saturation: number = 1.0;
  export let followMouse: boolean = true;
  export let mouseInfluence: number = 0.1;
  ...
  onMount(() => {
    const glRenderer = new Renderer({ dpr: Math.min(window.devicePixelRatio, 2), alpha: true });
    renderer = glRenderer;
